...
#                       (run_gh19_s3_pulse_pipeline_mutant.py's BYPASS_CORRECTION: SAME ANCHOR) and it
#                       is the neutralised arm of the IMPROVEMENT criterion: with the correction gone,
#                       the improvement columns must come in short and the gate must say so BY NAME
#                       (`PRB-FAIL-IMPROVEMENT`). Reusing S3's exact anchor rather than writing a
#                       second one is deliberate -- the claim being controlled is the same claim, so a
#                       second encoding of it could only drift.
#                       The replacement is NOT verbatim S3's, in one respect, and the difference is
#                       confined to text that cannot execute: see the note above BYPASS_REPLACEMENT.
#
...
# --- improvement-bypass --------------------------------------------------------------------------
# The ANCHOR is verbatim S3's. The REPLACEMENT is not, and the whole of the difference is that it
# does not re-emit the body that its own `return 0.0;` makes unreachable.
#
# Why that matters here and did not in S3: S3's replacement keeps the guard line and the real body
# below the return, so every statement in the body is dead code. S3 got away with it because its
# mutated compile happens only inside the ubuntu-only `probe-gate` job. This slice turned the same
# mutation into a CMake TARGET, so the every-push FOUR-PLATFORM build compiles it -- and MSVC reports
# unreachable code at /W4 (C4702), which /WX makes fatal (C2220). Two other ways out would each have
# cost something this slice's criteria rest on: an MSVC-only `/wd4702` (the repo has ZERO warning
# suppressions in non-third_party files, and it would make the arms' "identical flags" property false)
# and moving the compile out of the matrix (hiding the red, and leaving the control arm uncompiled on
# four platforms). So the dead body is not emitted -- and the deletion is ASSERTED by
# `strip_dead_body()`, not assumed, because a cut that takes one statement too many would turn this
# control arm into a different experiment while still building and still passing.
#
# The parameters are UNNAMED for the second-order reason: with the body gone, nothing refers to them,
# and /W4 reports an unreferenced formal parameter (C4100) -- so fixing C4702 the obvious way would
# just trade one fatal warning for another.
BYPASS_ANCHOR = (
...
BYPASS_REPLACEMENT = (
    ...
    "  // The statements that follow this return in the shipped header (the `dt` guard, the capped\n"
    "  // kernel width, and the two-edge residual) are not repeated below it: below it they are\n"
    "  // unreachable, MSVC /W4 reports unreachable code as C4702 and /WX makes that fatal, and\n"
    "  // removing code that cannot execute cannot change what this arm computes. What they compute\n"
    "  // is precisely what this control arm exists to take away.\n"
    "  return 0.0;  // CONTROL: the two-edge correction bypassed entirely"
)
# The landmarks `strip_dead_body()` reconciles the cut against. `\n` + BYPASS_BODY_START must be the
# very next text after the control return, and BYPASS_TAIL must end the region: together they pin the
# cut to "this function's body, from its first statement through its closing brace" rather than to
# "everything from here to the end of the file".
BYPASS_BODY_START = "  const double w = (dt < kPolyblepMaxDt) ? dt : kPolyblepMaxDt;\n"
BYPASS_TAIL = (
    "  return polyblepResidual(t, w) - polyblepResidual(b, w);\n"
    "}\n"
)
# Statements the shipped body contains, each expected exactly once inside the cut. Naming them is what
# makes the refusal actionable: if the product body is edited and this tool is not updated, the cut
# stops matching the body it claims to be removing and the build fails HERE rather than staging a
# control arm that quietly removes something else.
BYPASS_BODY_CONTAINS = (
    "  double b = t - duty;\n",
...
def _cut_region(text, body_start, tail):
    """(start, end) of `body_start` .. the end of the first `tail` after it, or None."""
...
def strip_dead_body(staged, pristine):
    """Remove the body the bypass's `return 0.0;` made unreachable. Returns (text, removed), or None
    after printing an INVALID line naming the reason.

    Every clause here exists because the failure it prevents produces a control arm that BUILDS and
    PASSES while measuring a different experiment than the one the report describes. The cut is not
    "from the return to the end of the file" and not "to the next line that looks like a close": the
    region is pinned at BOTH ends by named statements of the shipped body, checked against the shipped
    text as well as the mutated one, and it has to begin at the statement the return makes unreachable
    or the word unreachable is not true.
    """
    cut_start = staged.find(BYPASS_REPLACEMENT)
    if cut_start < 0:
        print("INVALID: the improvement-bypass replacement is not present in the mutated text")
        return None
    cut_start += len(BYPASS_REPLACEMENT)
    if not staged[:cut_start].endswith(BYPASS_REPLACEMENT):  # unreachable; stated, not assumed
        print("INVALID: the improvement-bypass cut does not start at the replacement's end")
        return None
    if not staged.startswith("\n", cut_start):
        print("INVALID: the bypass's return is not the end of its line")
        return None
    tail_hits = staged.count(BYPASS_TAIL, cut_start)
    if tail_hits != 1:
        # Exactly one, so the region cannot run past this function's closing brace into whatever
        # follows it: a second closing brace after the cut start means the landmarks straddle two
        # functions and the "remove the body" reading is no longer the one being implemented.
        print("INVALID: the closing-brace landmark occurs %d times after the bypass cut (expected "
              "exactly 1)" % tail_hits)
        return None
    cut_end = staged.index(BYPASS_TAIL, cut_start) + len(BYPASS_TAIL)
    removed = staged[cut_start:cut_end]
    if not removed.endswith(BYPASS_TAIL):
        print("INVALID: the improvement-bypass cut does not end at the function's closing brace")
        return None
    if "\n}" in removed[:-len(BYPASS_TAIL)]:
        print("INVALID: the improvement-bypass cut spans a closing brace before its own end")
        return None
    for literal in (BYPASS_BODY_START,) + BYPASS_BODY_CONTAINS:
        n = removed.count(literal)
        if n != 1:
            print("INVALID: the improvement-bypass cut does not contain the shipped body statement "
                  "%r exactly once (found %d) -- the body has drifted from what this tool removes"
                  % (literal, n))
            return None
    # Above the body's first statement, the cut may only be discarding blank lines and comments.
    # This is the clause that makes "the remainder of the function body" checkable rather than
    # asserted: if an edit ever leaves real code between the bypass's return and that first statement,
    # the deletion would silently take a computation with it and the arm would stop being
    # product-minus-correction.
    for line in removed[:removed.index(BYPASS_BODY_START)].split("\n"):
        if line.strip() and not line.lstrip().startswith("//"):
            print("INVALID: the improvement-bypass cut would discard %r, which is neither blank, a "
                  "comment, nor part of the shipped body" % line)
            return None
    # ...and the same region in the SHIPPED text, derived through the anchor rather than through the
    # replacement, must be the same bytes. This is what says the mutation inserted lines and moved
    # nothing: if an edit ever makes the replacement swallow part of the body, the two derivations
    # stop agreeing and this refuses instead of removing less than it reports removing.
    anchor_end = pristine.find(BYPASS_ANCHOR)
    if anchor_end < 0:
        print("INVALID: the improvement-bypass anchor is not present in the shipped text")
        return None
    anchor_end += len(BYPASS_ANCHOR)
    if not pristine.startswith("\n", anchor_end):
        print("INVALID: the anchored guard in the shipped text is not the end of its line")
        return None
    shipped = _cut_region(pristine, BYPASS_BODY_START, BYPASS_TAIL)
    if shipped is None or pristine[anchor_end:shipped[1]] != removed:
        print("INVALID: the region cut from the mutated text is not byte-identical to the shipped "
              "text's same region (%d vs %d bytes)"
              % (len(removed), -1 if shipped is None else shipped[1] - anchor_end))
        return None
    return staged[:cut_start] + "\n}\n" + staged[cut_end:], removed
...
def stage(repo_root, shadow_root, mutation, quiet=False):
    ...
    dead_removed = 0
    if mutation == "improvement-bypass":
        stripped = strip_dead_body(staged, text)
        if stripped is None:
            return None
        staged, removed = stripped
        dead_removed = len(removed)
    ...
    if not quiet:
        ...
        if mutation == "improvement-bypass":
            print("PRB-SHADOW dead_body_removed_bytes=%d unreachable_under_bypass=yes" % dead_removed)
