...
> **（未合入 · 本轮切片裁决 2026-09-14）GH#19 S0：波形映射与双侧 PWM 生产接入**（task #117；隔离分支 `feat/19-morph-pwm-product`，**未 push、未合、未判 MET**。本段不修改上面任何基于 `origin/main` 的事实快照，也不改变任何阶段的 MET / NOT MET；GH#19 **仍 OPEN**。）
> **① 波形选择 = 单一连续 morph 控件（软件暂定裁决，非硬件事实）**：节点环 `saw / 反相 saw / 正弦 / 三角 / 脉冲`，坐标 `{0, .25, .5, .75, 1}`（`kRingEqual`），相邻节点以**线性等权（partition of unity）**混合，复用既有 morph 参数 ID / 取值域 `0..1` / 默认 `0.5` 与既有平滑；**无选择器、无独立第二控制量**。本轮只裁定**连续行为**。面板/硬件是否另有离散选波**仍未证**（task #115 已撤回全部硬件性结论）。
> **② 默认音色变更（有意）**：波形默认 **三角 → 正弦**（morph 默认 `0.5` 落在纯正弦节点）；**旧的非默认 morph 档位音色随之改变**。**wire 字节、参数 ID、默认值本身均未变。** 三角↔反相三角中点静音保持实测样态，**不隐藏补增益**。
> **③ PWM 传递（软件暂定，非硬件事实）**：`effectiveDuty = clamp(basePW + depth * cvVolts / 10, 0.001, 0.999)`（+CV 抬高占空比；±5 V @ depth 1 贡献 ±0.5）。**该比例不来自硬件事实。** 无连线读 0；`depth = 0` 严格不变；外部调制**不参与**既有旋钮平滑（每采样读实际 graph CV）；`basePW` 的规范值不被覆写；复用现有 sink 解析/回读机制，**不绕过 graph 调度器**。
> **④ state disposition 改判**：`vco_a_pwm`(8) / `vco_b_pwm`(30) 由 `transfer_unavailable` → `applied_to_dsp` ⇒ `applied_to_dsp` **183 → 185**、`transfer_unavailable` **2 → 0**、总数 **345 不变**；冻结描述符值与 `fieldEvidence` **未动**。
>
...
## 1b. git 事实（2026-08-26 核实 · **本节是历史快照，当前 tip 见文首横幅**）

> ⚠️ 本节记的 `baf1e11` / `feat/p0-full-registry` / PR#2 均已成历史，**当前 `origin/main` 以文首横幅为准**。
> 保留它是因为**它立的规矩仍然有效**：问「main 动没动」时，**权威是 fetch 后的 `origin/main`，不是本地 ref**。
...
`full coverage (--require-full)` **仍按设计红**：既有 **12 项** `keyboard.*` 覆盖缺口（8 条非标量 must-gap + 4 条无值域；`rogue=[]` / `newRogue=[]`，无新增），**按 PR 逐次批准例外，非永久豁免**。

**下一刀（建议；执行前需总监确认）**：按 GH#19 分段先做 **S0（pwm 产品可达性）**，再走 **S3 / S6**；**S0 未解前不得声称 PWM 抗混叠可达** —— `vco_a_pwm` / `vco_b_pwm` 缺的是**运行时消费者**（`transfer_unavailable` 余下的正是这两条），属**可达性**问题、不是抗混叠问题（task #108）。**S4 需 owner 先裁「是否允许过采样」**；S5 收口待复核。
...
2. **无反相门**：依据＝`OUTS VOLTAGE SPECIFICATION`（L151-161）逐条列死各输出电压范围，
   **所有门类输出一律单极正**（EG 0…8V／ENV FOLLOWER GATE 0…+8V／5 STEP SEQ GATE 0…+10V）；
   ...
   （✅ **2026-08-31 已闭合**：`PULSERL` 行后置 `L` 为 **manual 笔误**——即 PULSER，其周期输出对应 registry `sequencer.clock_out`，**双极 −10…+10V 已确认**；原先"命名/电气对应尚未核清"的 raw-evidence conflict 已解，`clock_out` 计入已确认实现范围。），
...
### #39 通过收口 + 一个新洞挡住 P3 恢复（2026-08-27，msg `c3630230`，head `336bf4f`）

| 我的突变 | 结果 |
|---|---|
| 产品路径 `drone[q]=0.5`（绕开 bank） | **红 5 条**（含新增的"来自 bank"、"VARIES"） |
| **只**把 `setDroneTune` 改 no-op | **恰好红 1 条**（对应那条） |
| 恢复 | 45/45，tree 净 |

**动一个旋钮只红一条 ⇒ 判据有分辨力，不是一动一片红。**
`droneChannel()` 读 `chIn_[...]`（真喂给 mixer 的那份）⇒ **钉在被执行的数据上，这次 @Pi 自己先做对了。**

### ⚠️ P3 仍不能恢复：NEW drone 3/6 在产品路径上是哑的
```cpp
chIn[kChannelDrone3] = 0.0;   chIn[kChannelDrone6] = 0.0;
```
且 `machine_runtime.h` 中 **`SchmittOsc` / `NoiseSource` 出现次数 = 0**
⇒ **P3-② 的 NEW 声部根本没接进产品路径；六个 drone 声部有两个在机器上静音。**
而 P3 出口第 1 条要求「除 dual effector 外的**整条**信号链可演奏」。

**与 A03 同病、换个模块**：零件（`drone_noise.h`/`schmitt_osc.h`）在盘上、单测过，**产品不用它**。
⇒ **task #44**（已派 @Pi，优先于 #40）：接入 NEW drone 3/6，判据先红后修
（①来源==独立 SchmittOsc/NoiseSource 预期、非 0 非 classic 复制 ②必须 VARIES ③按 `01:43` 的 NEW 控件绑定）。
**验收：把 drone 3/6 改回 `0.0` 必须红。**
...
### 📐 P4 切分方案（2026-08-27 我拟，等 #46 后启用；**不与 P5 撞车**）

**问题**：P4 出口是「相同控制序列**从面板、电脑键盘和 MIDI** 进入后，产生相同的内部 CV/gate/clock」，
但**面板**由 P5 建。若照字面等面板，P4 就反过来依赖 P5，倒置阶段顺序（与我 8/26 驳 Codex A03 同一逻辑）。

**切法：适配器吃「抽象输入」，不吃 UI。**
- **P4-A（先做）**：`InputStateMachine` 进产品运行时；三个 framework-free 输入源产出 `ControlEvent`：
  ①**指针/触摸**＝抽象事件（触摸片索引 + pressure），**不含命中测试**；
  ②**电脑键盘**＝按键→音高/gate；
  ③**MIDI**＝合成 MIDI 字节流（velocity/aftertouch→pressure、MIDI clock→external clock、CC learn→原控件/CV，`03 §4` 边界）。
  **三路等价必须在产品运行时上验，不接受测试内 stub**（`test_input_equivalence.cpp` 现在那种不算）。
- **P4-B**：`PressureOutlet`（keyboard behaviour）、`PressureNormalizer`（归一化校准）、`ArpSeq` 接入产品路径。
- **留给 P5/host**：面板命中测试 → 指针适配器的真实坐标；真实 MIDI 设备 → MIDI 适配器
  （host 现 `PLUG_DOES_MIDI_IN 0`）。**这两条是"接线"，不是 P4 的能力。**

**依据**：`03 §4` 已把 MIDI 定义为**适配器**（「UI/MIDI 只改变原面板已有参数、gate、clock 与 CV 关系；
通过有界消息通道交给音频线程」）⇒ **适配器与设备/UI 可分离**，P4 验能力、P5 接设备。

**P4 出口判定口径（现在写死，避免将来又"一次挖一个洞"）**：
三路**抽象输入源**在产品运行时产生**逐位相同**的 CV/gate/clock；**任一路绕开 state machine ⇒ 必红**。
面板真实坐标与真实 MIDI 设备属 P5 接线，**不计入 P4 的门**。
...
2. **P3 的 GH#11 子项已 FIXED-CANDIDATE（2026-08-31 收口）**：design/06 §3 的 L2「控制与路由」把 LFO×2、EG×2、joystick、物理 5-step 列为必需能力，P0–P8 此前没给它们 DSP owner，正确归属为 **P3 固定声音核心的"可跳线控制源"子项**（**不是** P4 keyboard）——design/06 P3 新增第 6 项（Envelope A/B、LFO A/B、Joystick、物理 5-step），并加硬门（框架无关、无堆、逐 sample 生产实现；canonical factory；被 SynthRuntime/PatchGraph 按 registry JackId 真实消费；public 输出 virtual volts；forward 行为测试 + 旧错红的 negative；仅 class/registry/结构扫描不当"有实现"）。**截至 head（GH#11 收口片）这六项已有生产实现与消费**：canonical `MachineRuntimeDefinition` 规范生产实例 + 35 参数经 public 参数事件路径 dispatch + 11 输出以 virtual volts 出版 + 被真实 PatchGraph 汇点消费（`vcfCvReadbackL/R`、`droneChannel`、EG gate/ENV、LFO/joy 输出、seq cv/gate/clock_out）。**`maxBlockSize/maxResources=0` 是宿主 prepare/resources 未填的 UNPREPARED/UNSPECIFIED 哨兵（GH#4/#10 主机准备/资源依赖），不是 GH#11 缺口** —— 六个控制源本身在本 head 已实现并被消费。
...
**证据口径（只写证据允许的验收，未发明硬件常数）**：LFO 公开 CV 单极 0..+10V（manual OUTS SPEC，confirmed，保留单极性）／Envelope ENV 0..8V（confirmed），VCA-CV 极性/transfer provisional，ATT/DEC/RLS 曲线与真实秒数、HOLD/SELF-GEN transfer unverified／Joystick 两路 −10..+10V（confirmed），机械 taper/center/offset unverified／物理 5-step：step-CV 0..+5V（`cv_out`，confirmed）、gate 0..+10V（`gate_out`，confirmed）。**（2026-08-31 GH#11 收口）**：① `PULSERL` 行后置 `L` = **manual 笔误**（即 PULSER），其周期输出 `sequencer.clock_out` **双极 −10…+10V 已确认**（polarity bipolar / nominal −10..+10，FieldEvidence 定性项 confirmed），原先"命名/电气对应尚未核清"的 raw evidence conflict 由此闭合并计入实现范围；② **PULSER norm→Hz 端点/taper = Provisional**：`hz=0.05·400^n`（`kFiveStepPulserMinRateHz=0.05`、`kFiveStepPulserLogBase=400`），norm∈[0,1]→0.05..20 Hz，norm=0 **不**宣告停止（0.05 Hz≈20s/步），域外/非有限→fail-closed 0.0（非法值 `invalid_value`）；③ **脉冲宽度 = Provisional 单 sample**（每上升沿 `clock_out` 恰一个 +10V，其余 −10V）；④ ⚠️ **`ext_clock_in` 电气/阈值本次收口有意保持不变**（未反推、未改 descriptor 码，仍 unverified）；通电 playhead unverified/provisional；不得凭空发明 reset。`EnvelopeFollower` 是 preamp 相关的 **L2 控制与路由**检测器/控制源（实现归属 P3 第 4 项），**不是** Envelope A/B、不作 GH #11 控制源之一。
