...
- **生产 vco.h 只读核对（@Codex 1a8ed7f2 Gap 2 + 104cbc28 容差分拆）**：`gen_gh19_blamp.py --check-prod core/include/lunar24/core/vco.h` 读取**实际生产头文件**，从其自身符号（`kN_blamp = <int>`、`a / L * kN_blamp`、`kLut[] = {`…`};`）解出 L/N 配置，再把生产 kLut 逐点与解析 g 比对。**两套独立容差**（@Codex 104cbc28 指出：原把插值容差 `2e-3` 当系数容差用，corner +0.001 仍 PASS）：①**网格系数一致性 `coeff_tol=1e-9`**——按 9 位生成精度比对（比较 round(g,9) 生成值），任一生产系数改错即红；②**插值误差 `interp_tol=2e-3`**——(L,N) 网格几何的独立口径，不用于判系数正确性。**生产 L=8/N=256 按既定配置核（非"读到即接受"）**：kN_blamp+1 必须=256、support L 必须=8，否则红。**正样本**：`--check-prod` 对集成后 vco.h = **PASS**（max|production−generated| = **0.000e+00** ≤ coeff_tol 1e-9，L=8 N=256，interp-err 9.7742e-04 ≤ 2e-3）+ corner/边界断言通过；**负样本**：corner 0.810569469 → 0.811569469（+0.001）→ **RED**（diff 1.000e-03 > 1e-9，exit 1）；kN_blamp 255 → 127（N=128≠256）→ **RED**（config mismatch，exit 1）。**已注册 CTest `gh19_blamp_lut_gate`**（跑 `--check-prod`，默认 coeff_tol 1e-9）——生产任一系数改错该项必红，CI 强制。此处不再沿用（但保留）旧的生成物一致性（自解析自己 emitted 文本）；它作为解析/插值/常量一致性的保留断言。
