...
8. **远端 MSVC 抓到一处本地看不见的真缺陷，已修**（这正是第 6 条「本地绿 ≠ MSVC 绿」的又一次实例）：PR #44 首个 head `9f169f4` 的 **windows-latest (cl)** 构建步失败，`test_machine_definition.cpp` 报 **2 × `error C3493`**（`'kFrames' cannot be implicitly captured because no default capture mode has been specified`，行 1303 / 1308）+ **9 × `error C2064`**（行 1339 / 1347 / 1349 / 1355 / 1363 / 1365 / 1373 —— 全部是 `sameA(...)` / `sameB(...)` 的调用点）。**根因单一**：`kFrames` 是**函数内** `constexpr`，而 `sameA` / `sameB` 用的是**空捕获** `[]`；clang / gcc 把对常量表达式的读视为不构成 odr-use ⇒ 本地三配置全绿，MSVC 判定为隐式捕获 ⇒ C3493；两个 `auto` lambda 的返回类型随之推导失败 ⇒ 每个调用点再报 C2064。**修法**：两处改为默认捕获 `[&]`（并就地留注释说明为什么不能用 `[]`），**不改语义**。修复后本地重跑受影响检查：`test_machine_definition` **1/1 通过**、`--label-exclude slow` **76/76**、13 条突变仍 **参考 4/4 GREEN + 13/13 各自具名 RED + 0 INVALID**。⚠️本机无法编译 MSVC，**该修法的 MSVC 有效性只能由远端 CI 确认**，不在本报告中当作已证。
